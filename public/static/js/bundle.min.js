...
var SUPPORTED_TIPCHAIN_PROTOCOLS = ["bit://" + B_MEDIA_PROTOCOL + "/", "b://", "bit://" + BCAT_MEDIA_PROTOCOL + "/", "https://bico.media/", "https://www.bitpaste.app/tx/", "https://memo.sv/post/", "https://dir.sv/category/", "https://dir.sv/link/", "https://bitstagram.bitdb.network/m/raw/"]; // Open Directory Bitcom Protocol
...
function get_bmedia_bitdb_query(txids) {
...
function get_bitdb_query() {
  ...
  var maxDepth = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 5;
  ...
  var query = {
    ...
    "q": {
      ...
      }, // add siblings arrays together
      {
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$confirmed_children"]
          }
        }
      ...
      // Crawl confirmed children (entries, votes, undos)
      {
        ...
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$unconfirmed_children"]
          }
        }
      }, {
        "$unwind": {
      ...
      // Crawl confirmed parents (tipchain needs path to root), so recursive join up
      {
        ...
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$confirmed_parent"]
          }
        }
      }, {
        "$unwind": {
      ...
      // Crawl unconfirmed parents (tipchain needs path to root), so recursive join up
      {
        ...
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$unconfirmed_parent"]
          }
        }
      }, {
        "$unwind": {
      ...
      // Crawl confirmed dir.sv media
      {
        "$lookup": {
          "from": "c",
          "localField": "dirsv_txid.txid",
          "foreignField": "tx.h",
          "as": "confirmed_dirmediatx"
        }
      }, {
        "$project": {
          "confirmed_dirmediatx": "$confirmed_dirmediatx",
          "object": ["$$ROOT"]
        }
      }, {
        "$project": {
          "object.confirmed_dirmediatx": 0,
          "object.dirsv_txid": 0
        }
      }, // including b:// files could make queries very large, so filter out data—we only need metadata
      {
        ...
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$confirmed_dirmediatx"]
          }
        }
      }, {
        "$unwind": {
      ...
      {
        "$addFields": {
          "b_txid": [// Split out different versions of b:// and bcat:// txids, works with bit:// general protocol too
          {
            "txid": {
              "$arrayElemAt": [{
                "$split": [{
      ...
      }, // including b:// files could make queries very large, so filter out data—we only need metadata
      {
        ...
        "$project": {
          "items": {
            "$concatArrays": ["$object", "$confirmed_bmediatx"]
          }
        }
      }, {
        "$unwind": {
...
function fetch_from_network() {
  ...
  var query = get_bitdb_query(category_id, cursor, limit);
...
function fetch_bmedia_from_network(txids) {
  var cursor = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  var limit = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 20;
  var results = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
  var query = get_bmedia_bitdb_query(txids, cursor, limit);
...
function processTipchain(processing, txpool) {
  ...
  try {
    for (var _iterator14 = txpool.filter(function (tx) {
  ...
  try {
    for (var _iterator15 = processing[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
...
function connect_to_bitdb_socket(category_id, callback) {
  ...
  var query = get_bitdb_query(category_id);
  var encoded_query = toBase64(JSON.stringify(query));
    ...
    value: function render() {
      ...
      if (this.props.item.type == "category") {
        ...
        return React.createElement("div", {
          ...
          className: "satoshis"
        }, satoshisToDollars(this.props.item.satoshis)), React.createElement(ReactMarkdown, {
          ...
          onPayment: function onPayment(msg) {
            ...
            setTimeout(function () {
              ...
              if (_this3.props.category.txid) {
                ...
                _this3.props.onSuccessHandler("Successfully added new category. Due to heavy demand we're caching the front page longer, so please wait up to a minute and refresh. We're working on resolving this ASAP!");
              }
            }, 3000);
          }
        }
...
function (_BaseList) {
  ...
  _createClass(SubcategoryList, [{
    ...
    value: function render() {
      ...
      return React.createElement("div", null, heading, categories && categories.length > 0 && React.createElement("div", {
...
function (_React$Component) {
  ...
  _createClass(OpenDirectoryApp, [{
    ...
    value: function getForks() {
      var txid = this.state.category.txid;
    ...
    value: function render() {
      ...
      if (path == "/faq") {
        ...
        var stats = this.buildItemSliceRepresentationFromCache(this.state.category.txid);
        ...
        if (!this.state.isError) {
          var filtered_items = this.filterOutDetaches(this.state.items);
      ...
      if (this.state.intro_markdown) {
        ...
        }, React.createElement("strong", null, "Note:"), " Due to high traffic from launch, new content won't appear instantly on the homepage after you submit\u2014please allow up to a minute and refresh. A fix is coming soon!"));
      }
      ...
      }))), path == "/" && intro, body, this.state.isLoading && this.state.items.length == 0 && loading, this.state.isError && error, React.createElement("hr", null), React.createElement("div", {
        className: "row"
    ...
    key: "buildItemSliceRepresentationFromCache",
    value: function buildItemSliceRepresentationFromCache(category_id) {
      if (!category_id) {
        return this.state.items;
      }

      var parent_categories = findParentCategoryChain(category_id, this.state.items);
      var items = new Map();
      var _iteratorNormalCompletion3 = true;
      var _didIteratorError3 = false;
      var _iteratorError3 = undefined;
      ...
      try {
        for (var _iterator3 = parent_categories[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          var _txid = _step3.value;

          var _parent = findObjectByTX(_txid, this.state.items);

          items.set(_txid, _parent);
        }
      } catch (err) {
        _didIteratorError3 = true;
        _iteratorError3 = err;
      ...
      while (subcategory_txids.length > 0) {
        ...
        txids.add(txid);
        var found_children = findChildrenOfParentCategory(txid, this.state.items);
        var _iteratorNormalCompletion4 = true;
        var _didIteratorError4 = false;
        var _iteratorError4 = undefined;

        try {
          for (var _iterator4 = found_children[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
      ...
      var children = this.state.items.filter(function (i) {
        if (txids.has(i.txid)) {
          return true;
        }
    ...
    key: "buildRawSliceRepresentationFromCache",
    value: function buildRawSliceRepresentationFromCache(category_id) {
      ...
      var items = this.buildItemSliceRepresentationFromCache(category_id); // Grab parents first, since we don't want their children (undos, votes) we do them separate
      ...
      try {
        for (var _iterator6 = changelog[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
          var log = _step6.value;

          if (txids.indexOf(log.txid) !== -1) {
            filtered_changelog.push(log);
          } else if (txids.indexOf(log.data.s3) !== -1) {
            console.log("FOUND LOG", JSON.stringify(log.data, null, 4));
            filtered_changelog.push(log); //            }
          }
        }
      } catch (err) {
      ...
      console.log("FILTERED", filtered_changelog);
      /*
      const all = [];
    ...
    value: function didUpdateLocation() {
      ...
      if (path == "/faq") {
        ...
        if (parts.length > 0) {
          var entry_id = parts[2]; // Hacky...but will rip this all out when planaria is built

          if (CACHED_HOMEPAGE) {
            var txpool = processOpenDirectoryTransactions(CACHED_HOMEPAGE);
            var results = processResults(CACHED_HOMEPAGE, txpool);
            var entry = findObjectByTX(entry_id, results);
        ...
        if (path == "/") {
          ...
          var _parts2 = path.split("/");

          if (_parts2.length > 0) {
            if (_parts2[1] == "category") {
              category_id = _parts2[2];
            } else {
              this.setState({
                "isError": true,
                "isLoading": false
        ...
        var cached = this.state.cache[category_id];
        category = {
          "txid": category_id,
          "needsdata": true
        };
        ...
        if (cached) {
          items = cached;
          var cachedCategory = findObjectByTX(category_id, cached);
    ...
    value: function networkAPIFetch() {
      ...
      this.setState(state);
      setTimeout(function () {
        var category_id = _this8.state.category ? _this8.state.category.txid : get_root_category_txid();
        fetch_from_network(category_id).then(function (rows) {
          var txpool = processOpenDirectoryTransactions(rows);
          var results = processResults(rows, txpool);

          var success = _this8.checkForUpdatedActiveCategory(results);

          var cache = _this8.state.cache;
      ...
      this.socket.onmessage = function (e) {
        try {
          ...
          if ((resp.type == "c" || resp.type == "u") && resp.data.length > 0) {
            ...
            if (rows.length > 0) {
              console.log("handled new message", resp); //this.insertNewRowsFromNetwork(rows);
            }
          }
        } catch (e) {
          console.log("error handling network socket data", e.data);

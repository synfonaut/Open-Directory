  ...
  _createClass(OpenDirectoryApp, [{
    ...
    value: function didUpdateLocation() {
      ...
      if (path == "/faq") {
        ...
        var cached = this.buildItemSliceRepresentationFromCache(category_id);
        category = {
          "txid": category_id,
          "needsdata": true
        };
    ...
    value: function insertNewRowsFromNetwork(new_rows) {
      console.log("INSERTING NEW ROWS", new_rows);
      var category_id = this.state.category ? this.state.category.txid : get_root_category_txid();
      var rows = this.state.changelog.concat(new_rows);
      var txpool = processOpenDirectoryTransactions(rows);
      var results = processResults(rows, txpool);
      var success = this.checkForUpdatedActiveCategory(results);
      this.setState({
        "isError": !success,
        "txpool": txpool,
        "items": results,
